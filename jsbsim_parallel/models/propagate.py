...
class VehicleState:
    def __init__(self, queue_length: int = 10, *, device, batch_size: Optional[torch.Size] = None):
        ...
        self.qAttitudeLocal = torch.zeros(*size, 4, dtype=torch.float64, device=device)  # Assuming quaternion [w, x, y, z]

        # Orientation of vehicle relative to inertial ECI frame (quaternion)
        self.qAttitudeECI = torch.zeros(*size, 4, dtype=torch.float64, device=device)  # Assuming quaternion [w, x, y, z]

        # Placeholder for quaternion derivative (not fully specified in provided code)
        self.vQtrndot = torch.zeros(*size, 4, dtype=torch.float64, device=device)
    ...
    def add_qtrndot(self, new_qtrndot: torch.Tensor):
        self.update_queue("dqQtrndot", new_qtrndot)
